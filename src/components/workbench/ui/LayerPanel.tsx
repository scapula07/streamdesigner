...
export default function LayerPanel({workspace}: {workspace: any}) {
  return (
    <nav className="flex flex-col items-center py-2 px-1 bg-white rounded-2xl shadow border border-gray-200 gap-1 mt-4 ml-2 select-none" style={{ width: 56 }}>
      {icons.map((icon, idx) => (
        <div key={icon.key} className="relative group">
          <button
            className={`w-12 h-12 flex items-center justify-center rounded-xl transition hover:bg-gray-100 ${icon.key === "chat" ? "bg-indigo-50 border border-indigo-200" : ""}`}
